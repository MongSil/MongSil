package kr.co.tacademy.mongsil.mongsil;

/**
 * Created by ccei on 2016-08-01.
 */
public class NetworkDefineConstant {
    public static final String APP_KEY = "b97acc1a-399a-3ce5-b55a-f6bb4bf021f9";
    public static final String HOST_URL = "http://52.78.101.201";
    public static final String PORT_NUMBER = ":3000";
    public static final String SERVER_POST
            = HOST_URL + PORT_NUMBER + "/post";
    public static final String SERVER_USER
            = HOST_URL + PORT_NUMBER + "/users";
    public static final int POST_OK = 1;
    public static final int POST_FAIL = 2;
}
